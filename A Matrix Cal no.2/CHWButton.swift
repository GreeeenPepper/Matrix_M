...
class CHWButton: UIButton {
    ...
    convenience init(count: Int,frame: CGRect , color: UIColor?) {
        ...
        self.addLabel()
        super.addTarget(self, action: #selector(CHWButton.startCountDown), for: UIControlEvents.touchUpInside )
    }
...
//    开启定时器
    func startCountDown() {
        self.timer = Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(CHWButton.countDown), userInfo: nil, repeats: true)
...
//    旋转变小动画
    func rotateAnimation() {
        ...
        timeLabel.layer.add(shakeAnimation, forKey: nil)
        self.layer.addSublayer(timeLabel.layer)
