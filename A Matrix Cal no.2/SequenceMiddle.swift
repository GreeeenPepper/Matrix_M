...
import UIKit
class SequenceMiddle:UIViewController{
    ...
    func roundaddone(){
        disablekeyboard()
        self.Title_Score.text = "Round:" + String(Round_Time) + "/15"
    }
    var BrickNo:Int = 1
    ...
    func randomlyselectinSequence(){
        ...
        if(Brick_Already_Select == Round_Time + 2){
    ...
    @IBAction func Press_M11(_ sender: AnyObject) {
        if(BrickSelect[0][0] != BrickSelectedNo){
            ...
            BrickSelectedNo += 1
            if(BrickSuccessfullySelect == Round_Time + 2){
                if(Round_Time == 15){
